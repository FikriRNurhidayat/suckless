...
enum { SchemeNorm, SchemeSel, SchemeDorm }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : occ & 1 << i ? SchemeNorm : SchemeDorm]);
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.dormbordercolor", dormbordercolor);
        XRDB_LOAD_COLOR("dwm.dormbgcolor", dormbgcolor);
        XRDB_LOAD_COLOR("dwm.dormfgcolor", dormfgcolor);
...
monocle(Monitor *m)
{
	...
	if (n > 0) /* override layout symbol */
		snprintf(m->ltsymbol, sizeof m->ltsymbol, "Stack %d", n);
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, (bh - systrayiconsize) / 2, systrayiconsize, systrayiconsize);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
